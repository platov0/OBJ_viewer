#ifndef MODEL_INDICES_BUFFER
#define MODEL_INDICES_BUFFER
#include <vulkan/vulkan.h>
#include <vector>
#include <stdexcept>
class ModelIndicesBuffer
{
public:
	void CreateIndicesBuffer(VkPhysicalDevice& physical_device, VkDevice& device, VkQueue& graphic_queue, uint32_t graphics_family_index, VkCommandBuffer& transit_command_buffer, const void* data, uint32_t buffer_size)
	{
		...
		CreateBuffer(physical_device, device, buffer_size, buffer, buffer_memory, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
